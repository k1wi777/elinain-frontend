...
# Checkpoints definidos en .rei/docs/project/verification.md (V1–V4).
# Si añades o cambias un checkpoint allí, actualízalo también aquí.
...
run_check "Formato (V1)" npm run format:check
run_check "Lint (V2)"    npm run lint
run_check "Tipos (V3)"   npm run typecheck
run_check "Tests (V4)"   npm test
